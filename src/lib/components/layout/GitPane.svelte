<script lang="ts">
  ...
  async function initRepo() {
    ...
    try {
      ...
      showToast("Versionamento iniciado", "success");
      await refresh();
    } catch (e) {
      showToast(`Erro ao iniciar Git: ${e}`, "error");
    } finally {
      busy = false;
    }
  }
  ...
  async function commit() {
    ...
    try {
      ...
      showToast("Versão salva", "success");
      message = "";
      await refresh();
    } catch (e) {
      const msg = String(e);
      if (/identity|user\.name|user\.email|please tell me who you are/i.test(msg)) {
        showToast("Configure git user.name/user.email primeiro", "error");
      } else if (/nothing to commit/i.test(msg)) {
        showToast("Nada novo para salvar", "info");
      } else {
        showToast(`Erro ao salvar versão: ${msg}`, "error");
      }
    } finally {
      busy = false;
    }
...
<div class="flex h-full flex-col">
  <div class="flex h-10 shrink-0 items-center gap-2 border-b border-border px-3">
    ...
    <span class="text-sm font-medium">Versões (Git)</span>
  ...
  <div class="flex-1 overflow-auto p-3">
    {#if loading}
      <div class="flex items-center justify-center gap-2 py-8 text-sm text-text-secondary">
        <Loader2 size={15} class="animate-spin" /> Lendo repositório…
      </div>
    {:else if !$currentVaultPath}
      <div class="px-2 py-8 text-center text-sm text-text-secondary">Abra um vault primeiro.</div>
    {:else if status && !status.is_repo}
      <div class="rounded-xl border border-border bg-bg/30 p-4 text-center">
        ...
        <p class="mt-3 text-sm text-text-secondary">
          Este vault ainda não tem versionamento. Inicie pra salvar versões e ver o histórico.
        </p>
        <button
          onclick={initRepo}
          disabled={busy}
    ...
    {:else if status}
      ...
      <div class="rounded-xl border border-border bg-bg/30 p-3">
        <div class="mb-2 flex items-center gap-1.5 text-xs font-medium text-text-secondary">
          <FileDiff size={13} />
          {status.clean ? "Nenhuma alteração pendente" : `${status.changed.length} alteração(ões)`}
        </div>
        {#if !status.clean}
        ...
        <input
          ...
          placeholder="Descreva a versão (opcional)…"
      ...
      {#if log.length === 0}
        <div class="px-2 py-4 text-center text-xs text-text-muted">Nenhuma versão salva ainda.</div>
      {:else}
