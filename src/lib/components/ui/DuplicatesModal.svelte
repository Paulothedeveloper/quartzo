...
{#snippet body()}
  <p class="text-xs leading-relaxed text-text-secondary">{$t("dup.hint")}</p>
...
{#if $duplicatesOpen}
  {#if isMobile}
    <MobileScreen title={$t("dup.title")} icon={CopyX} onClose={close} footer={footerBtns}>
      {@render body()}
    </MobileScreen>
  {:else}
    <div
    ...
    >
      ...
      >
        ...
        <div class="flex min-h-0 flex-1 flex-col gap-3 overflow-auto p-3 pt-3">
          {@render body()}
        </div>
