...
{#if $insightsOpen}
  {#if isMobile}
    <MobileScreen title={$t("insights.title")} icon={Compass} onClose={close} subbar={tabsBar} pad={false}>
      <div class="p-2">{@render body()}</div>
    </MobileScreen>
  {:else}
    <div
    ...
    >
      ...
      <div
        class="qmodal-panel flex max-h-[78vh] w-full max-w-[580px] flex-col overflow-hidden rounded-2xl border border-border bg-surface/95 shadow-2xl backdrop-blur-xl"
